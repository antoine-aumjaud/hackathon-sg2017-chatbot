...
import com.tesobe.obp.account.Transaction.Tag;
...
public class TransactionAnnotationService {
	...
	public Tag addTag(String token, Transaction transaction, final String tagValue) {
		...
		if (existingTags != null) {
			List<Tag> matchTagz = existingTags.stream()
					.filter(tag -> tag.getValue().equals(tagValue.trim()))
					.collect(Collectors.toList());
			if (matchTagz.size() > 0)
				return matchTagz.get(0);
		...
		String taggingUrl = String.format(
				"%s/banks/%s/accounts/%s/owner/transactions/%s/metadata/tags", apiUrl,
				bankId, ownAccount.getId(), transaction.getId());

		HttpEntity<Tag> req = prepareAuthRequest(token, tagValue.trim());
		return restTemplate.exchange(taggingUrl, HttpMethod.POST, req, Tag.class)
				.getBody();
	}
	...
	public void deleteTag(String token, Transaction transaction, Tag tag) {
		...
		String taggingUrl = String.format(
				"%s/banks/%s/accounts/%s/owner/transactions/%s/metadata/tags/%s", apiUrl,
				bankId, ownAccount.getId(), transaction.getId(), tag.getId());
		HttpHeaders authHeaders = new HttpHeaders();
		String dlHeader = String.format("DirectLogin token=%s", token);
		authHeaders.add(HttpHeaders.AUTHORIZATION, dlHeader);
		restTemplate.exchange(taggingUrl, HttpMethod.DELETE,
				new HttpEntity<Void>(null, authHeaders), String.class);
	}
	...
	private String getBankId(String token, Account ownAccount) {
		return accountService.fetchPrivateAccounts(token, true).stream()
				.collect(Collectors.groupingBy(Account::getId)).get(ownAccount.getId())
				.get(0).getBankId();
	}
}

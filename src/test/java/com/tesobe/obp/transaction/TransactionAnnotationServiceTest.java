...
public class TransactionAnnotationServiceTest extends AbstractTestSupport {
	...
	@Autowired
	private TransactionAnnotationService transactionAnnotationService;
	...
	public void addTagOk() {
		...
		List<Transaction> transactions = monetaryTransactionsService
				.fetchTransactionList(authToken, ownAccount);
		...
		List<Tag> newTags = monetaryTransactionsService
				.getTransactionById(authToken, ownAccount, tx.getId()).getMetadata()
				.getTags();
	...
	// List<Account> accounts = accountService.fetchPrivateAccounts(authToken,
	// true);
	// Account ownAccount = accounts.get(0);
	// List<Transaction> transactions =
	// monetaryTransactionsService.fetchTransactionList(authToken, ownAccount);
	//
	// Transaction tx = transactions.get(0);
	// //tx.getMetadata().getTags().forEach(tag ->
	// transactionAnnotationService.deleteTag(authToken, tx, tag));
	// Tag tag = transactionAnnotationService.addTag(authToken, tx, "food");
	// List<Tag> txTags =
	// monetaryTransactionsService.getTransactionById(authToken, ownAccount,
	// tx.getId()).getMetadata().getTags();
	// Assert.assertTrue(txTags.contains(tag));
	//
	// transactionAnnotationService.deleteTag(authToken, tx, tag);
	// txTags = monetaryTransactionsService.getTransactionById(authToken,
	// ownAccount, tx.getId()).getMetadata().getTags();
	// Assert.assertTrue(!txTags.contains(tag));
	// }
